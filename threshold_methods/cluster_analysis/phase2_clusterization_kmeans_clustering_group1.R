...
#            with MULTIPLE thresholds, using OPTIMAL k (group1) from summary table
#
##########################################################################################
#ICES WKBENTH4
...
k_opt <- as.data.frame(fread(k_file)) %>%
  ...
  # keep only rows where k_group1 is valid
  filter(is.finite(k_group1), k_group1 >= 2)
...
run_from_kopt <- function(dat_all, k_opt,
  ...
  for (i in seq_len(nrow(combos))) {
    ...
    # k by dataset from k_opt (group1)
    k_by_dataset <- k_opt %>%
      filter(pressure_type == pt, thr_chosen == thr) %>%
      select(dataset_gear, k_group1) %>%
      distinct() %>%
    ...
    if (save_plots && nrow(dat_thr) > 0) {
      out_prefix <- paste0(out_prefix_base, "_", pt,
                           "_thr_", thr_tag(thr),
                           "_dataset_kmeancluster_group1_year_barplot")
...
fwrite(dataset.merged.thr_all,
       file.path(out_dir, "WKBENTH4_Type3_pressuretype_fixedpthr_kmeans_clusters_group1.csv"))

# optional: save cluster maps too
fwrite(cluster.maps.all,
       file.path(out_dir, "WKBENTH4_Type3_pressuretype_fixedpthr_kmeans_cluster_maps_group1.csv"))
