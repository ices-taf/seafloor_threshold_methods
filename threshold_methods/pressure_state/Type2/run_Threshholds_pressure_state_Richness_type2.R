# =======================================================================
# Pressure-state threshold estimation for Type 2 datasets: Richness
...
# and is therefore not used for count-based indicators modelled on a transformed continuous scale, such as
# richness.
#
...
#
# For richness, the response variable is transformed using log1p()
...
# -----------------------------------------------------------------------
# This script is specific to the richness indicator.
#
...
#   - Margalef diversity
#
# This script uses the richness field only.
#
# -----------------------------------------------------------------------
# Required packages
# -----------------------------------------------------------------------
...
out_table <- file.path(out_dir, paste0(base_name, "_TABLE_THRESHOLDS_RICHNESS.xlsx"))
...
if(is.na(col_rich)){
  stop("Could not find richness column. Check names(station_info).")
}

station_info$pressure_value <- num_clean(station_info[[col_p]])
station_info$richness  <- num_clean(station_info[[col_rich]])
station_info$richness_log <- log1p(station_info$richness)

station_info <- station_info %>%
  filter(is.finite(pressure_value), is.finite(richness_log)) %>%
  filter(pressure_value <= P_MAX)

msfd_data <- station_info %>%
  filter(is.finite(richness_log), is.finite(pressure_value))
...
tmp_gam <- tryCatch(
  gam(
    richness_log ~ s(pressure_value, k = 3),
    data = msfd_data,
    family = gaussian(),
    method = "REML"
  ),
...
if(!gate_ok){
  ...
  p_nv <- ggplot(NewData_CI, aes(x = x, y = y)) +
    ...
    labs(
      x = "Pressure",
      y = "Richness (log1p, normalised to P=0)",
  ...
  out_plot <- file.path(out_dir, paste0(base_name, "_PLOT_THRESHOLDS_RICHNESS.png"))
  ggsave(
    filename = out_plot,
    plot = final_plot,
    width = 10,
