# =======================================================================
# Pressure-state threshold estimation for Type 2 datasets: Margalef diversity
...
# and is therefore not used for continuous diversity indices such as
# Margalef diversity.
#
...
#
# For Margalef diversity, the response variable is modelled using a
# Gaussian GAM without transformation, following the Type 2 threshold
# framework used in the post-workshop analyses.
#
...
# -----------------------------------------------------------------------
# This script is specific to the  density-Margalef diversity indicator.
#
# The analysed field is:
#
#   "Rel Margalef div (dens)"
#
# The biomass-based Margalef field is not analysed in this example. However,
# the same workflow could be applied to the
# biomass-based Margalef indicator by changing the response-column name in
# the user settings / column-detection section.
#
# The same workflow should be applied separately to other indicators
# (e.g. SoS, total biomass, richness), using indicator-appropriate model
...
#   - Margalef diversity
#
# This script uses the biomass-based Margalef diversity field only.
#
# -----------------------------------------------------------------------
# Required packages
# -----------------------------------------------------------------------
...
out_table <- file.path(out_dir, paste0(base_name, "_TABLE_THRESHOLDS_MARGALEF_DENS.xlsx"))
...
col_margalef <- find_col(
  nms,
  "Rel Margalef div.*dens|Margalef.*dens|margalef.*dens"
)
...
if(is.na(col_margalef)){
  stop("Could not find density-based Margalef column (expected something like Rel Margalef div (dens)). Check names(station_info).")
}

station_info$pressure_value <- num_clean(station_info[[col_p]])
station_info$margalef  <- num_clean(station_info[[col_margalef]])

station_info <- station_info %>%
  filter(is.finite(pressure_value), is.finite(margalef)) %>%
  filter(pressure_value <= P_MAX)

msfd_data <- station_info %>%
  filter(is.finite(margalef), is.finite(pressure_value))
...
cat("Indicator: Margalef diversity (density-based)\n")
...
if(n_obs == 0){
  ...
  GAM_pvalues <- rbind(
    ...
    data.frame(
      Dataset = base_name,
      Indicator = "Margalef diversity (density-based)",
      n = 0,
      edf = NA,
      ref_df = NA,
      p_smooth = NA,
...
if(!gate_ok){
  ...
  p_nv <- ggplot(NewData_CI, aes(x = x, y = y)) +
    ...
    labs(
      x = "Pressure",
      y = "Margalef diversity (density-based, normalised to P=0)",
  ...
  out_plot <- file.path(out_dir, paste0(base_name, "_PLOT_THRESHOLDS_MARGALEF_DENS.png"))
  ggsave(
    filename = out_plot,
    plot = final_plot,
    width = 10,
