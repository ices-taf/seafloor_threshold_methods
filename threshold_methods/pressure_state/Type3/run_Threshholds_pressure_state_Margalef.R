# =======================================================================
# Pressure-state threshold estimation for Type 3 datasets: Margalef biomass (Rel Margalef div (biom))
# =======================================================================
...
#
# For Margalef, the response variable is fitted using a gaussian GAM
...
#   - Margalef diversity
#
# This script uses the "Rel Margalef div (biom)" field.
# -----------------------------------------------------------------------
# Required packages
# -----------------------------------------------------------------------
# - mgcv
...
thresholds_file <- file.path(
  output_folder,
  paste0(dataset_name, "_Margalef_thresholds.xlsx")
)
...
clamp01 <- function(z) pmax(0, pmin(1, z))

# (kept, used in bootstrap below)
state_from_ratio <- function(y_ratio) clamp01(1 - abs(1 - y_ratio))
# =========================
# 3) Clean core fields
# =========================
station_info$pressure_value <- as.numeric(gsub(",", ".", station_info$pressure_value))
station_info$margalef <- as.numeric(gsub(",", ".", station_info$`Rel Margalef div (biom)`))

station_info <- station_info %>%
  filter(is.finite(pressure_value), is.finite(margalef)) %>%
  filter(pressure_value <= 12)

# Merge Upper Bathyal names (kept)
station_info$habitat_type[
  station_info$habitat_type %in% c(
    "Upper bathyal sediment",
    "Upper bathyal sediment or Upper bathyal rock and biogenic reef"
  )
] <- "Upper Bathyal"

# Pressure grid for main predictions
...
for (msfd_bht in habitats) {
  ...
  # ---------------------------------------------------------
  tmp_gam <- tryCatch(
    gam(margalef ~ s(pressure_value, k = 3),
  ...
  # Use the SAME model for everything downstream (consistency)
  Mymodel_UBS <- tmp_gam
  ...
  p_nv <- ggplot(NewData_CI, aes(x = x, y = y)) +
    ...
    labs(x = "Pressure", y = "Margalef (normalized to P=0)", title = "Natural Variation") +
    theme_minimal()
  ...
  p_dc <- ggplot(NewData_CI, aes(x = x, y = y)) +
    ...
    coord_cartesian(ylim = c(0, 1)) +
    scale_x_continuous(limits = c(0, 12)) +
    labs(x = "Pressure", y = "Margalef (normalized to P=0)", title = "Detectable Change") +
    theme_minimal()
  ...
  out_plot_hab <- file.path(
    plots_folder,
    paste0(base_name, "_", safe_bht, "_PLOT_THRESHOLDS_Margalef.png")
  )  
  ggsave(
