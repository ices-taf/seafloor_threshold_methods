# =======================================================================
# Pressure-state threshold estimation for Type 3 datasets: Total biomass
# =======================================================================
...
#
# For total biomass, the response variable is log1p-transformed prior to
...
#   - Margalef diversity
#
# This script uses the total biomass field, modelled as log1p-transformed biomass.
# -----------------------------------------------------------------------
# Required packages
# -----------------------------------------------------------------------
# - mgcv
...
thresholds_file <- file.path(
  output_folder,
  paste0(dataset_name, "_Total_biomass_thresholds.xlsx")
)
...
station_info$total_biomass  <- as.numeric(gsub(",", ".", station_info$total_biomass))
...
for (msfd_bht in habitats) {
  ...
  p_nv <- ggplot(NewData_CI, aes(x = x, y = y)) +
    ...
    labs(x = "Pressure", y = "Total biomass (log1p, normalized to P=0)", title = "Natural Variation") +
    theme_minimal()
  ...
  p_dc <- ggplot(NewData_CI, aes(x = x, y = y)) +
    ...
    coord_cartesian(ylim = c(0, 1)) +
    scale_x_continuous(limits = c(0, 12)) +
    labs(x = "Pressure", y = "Total biomass (log1p, normalized to P=0)", title = "Detectable Change") +
    theme_minimal()
  ...
  out_plot_hab <- file.path(
    plots_folder,
    paste0(base_name, "_", safe_bht, "_PLOT_THRESHOLDS_BIOMASSLOG.png")
  )
  ggsave(
