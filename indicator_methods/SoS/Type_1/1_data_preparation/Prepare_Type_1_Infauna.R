# =========================================
# Type 1 INFAUNA (species by biomass) -> SoS ready
# =========================================
# Description:
# This script prepares Type 1 infauna datasets (species by biomass)
...
#      EE_Baltic, SE_Hafok, UK_Irish
#  - Detect Type 1 sheets:
#      BIO     = "Species information" (fallback to biological info / bio)
#      STATION = "Time series information" (fallback to station info)
...
#  - Merge BESITO from infauna lookup table (CSV)
#  - Assign default BESITO = 1 when missing (no match in lookup table)
#
# Input:
# - Type 1 datasets (species by biomass) from the repository data folder
# - BESITO infauna lookup table (external dependency)
#
# Output:
...
# - The BESITO infauna lookup table is not included yet and must be provided externally
# =========================================
# Required packages:
# - readxl
# - openxlsx
...
# ----------------------------
prefixes <- c("EE_Baltic", "SE_Hafok", "UK_Irish")
...
to_num_safe <- function(x){
  ...
  x <- gsub("???", "-", x)     # unicode minus# replace unicode minus with standard hyphen
  x <- gsub(",", ".", x)
  x <- gsub("\\s+", "", x)
  x <- gsub("[^0-9eE\\.\\-\\+]", "", x)
...
pick_biomass_col <- function(df){
  candidates <- c("outcome_value_biomass","outcome_value biomass","outcome_value","outcome_value_biomass_dry_weight")
...
# ----------------------------
# BESITO (DEFAULT = 1 when missing)
# ----------------------------
besito_file <- "SET_PATH_TO_EXTERNAL_BESITO_INFAUNA_FILE"
if(besito_file == "SET_PATH_TO_EXTERNAL_BESITO_INFAUNA_FILE"){
  stop("Please provide the path to the external BESITO infauna lookup table.")
}
besito_raw <- read.csv(besito_file, stringsAsFactors = FALSE)

names(besito_raw) <- tolower(trimws(names(besito_raw)))
...
besito_raw$besito  <- trimws(as.character(besito_raw$besito))
...
for(file_path in all_files){
  ...
  if(!is.na(bcol)){
    ...
    warning("No biomass column found in BIO. Biomass stays NA.")
  }
  ...
  if(!is.na(acol)){
    ...
    warning("No abundance column found in BIO. Abundance stays NA.")
  }
  
  # Units from BIO if present
  ...
  if(length(taxnames_missing) > 0){
    ...
    if(!is.null(worms_results) && nrow(worms_results) > 0){
      ...
      prepared_df$AphiaID[is.na(prepared_df$AphiaID) & !is.na(m)] <-
        aphia_map$AphiaID_map[m[is.na(prepared_df$AphiaID) & !is.na(m)]]
    }
  }
  ...
  # ----------------------------
  # Load STATION and merge by station_2
  # ----------------------------
  if(is.na(station_sheet)){
    ...
    if(is.na(st_station_col) || is.na(st_year_col)){
      ...
      tb_col <- find_col(station_raw, c("total_biomass","totalbiomass","total_biomass_dry_weight"))
  ...
  # ----------------------------
  # Merge BESITO (DEFAULT = 1 when missing)
  # ----------------------------
