# =======================================================================
# Prepare Type 3 INFAUNA dataset (NS_BE) for SoS calculation
# =======================================================================

# -----------------------------------------------------------------------
# Description
# -----------------------------------------------------------------------
# This script prepares the Type 3 infauna dataset NS_BEhabitats for
...
# in data structure compared to the general workflow, particularly in the
# definition of sampling units and station identifiers.
#
# A composite identifier (station_2), combining station, year, and
# replicate, is used to ensure consistent matching between biological
# and station-level information.

# -----------------------------------------------------------------------
# Main steps
# -----------------------------------------------------------------------
# - Read the NS_BEhabitats dataset from the repository data folder
...
# Notes
# -----------------------------------------------------------------------
# - This script is dataset-specific (NS_BEhabitats)
# - A composite identifier (station_2) is required for accurate matching
# - Differences in station structure prevent direct use of the generic script
...
# Process only the NS_BEhabitats dataset
all_files <- all_files[basename(all_files) == "NS_BEhabitats.xlsx"]
...
for(file_path in all_files){
  ...
  if(length(station_sheet) == 0){
    ...
    st_station_col <- find_col(station_raw, c("station","station_name","station_id","stationcode","stn"))
    ...
    if(is.na(st_station_col) || is.na(st_year_col) || is.na(st_rep_col)){
      stop(paste0(
    ...
    station_raw$station_2 <- paste(
      ...
      norm_rep(station_raw[[st_rep_col]]),
      sep = "_"
    )
  ...
  if(!all(c("aphiaid","besito") %in% names(besito_infauna))){
    stop("The infauna file does not contain required columns (e.g. 'AphiaID' and 'BESITO').")
    }
