# =======================================================================
# Prepare Type 3 INFAUNA datasets for SoS calculation
# =======================================================================

# -----------------------------------------------------------------------
# Description
# -----------------------------------------------------------------------
# This script prepares Type 3 infauna datasets for subsequent
...
# per dataset.
#
# This script is designed as a GENERAL workflow for infauna datasets.
# Due to differences in data structure across datasets (e.g. station
# identifiers, replicate handling, SAR format, or sheet structure),
...
#   (e.g. station + year + replicate; station_2)
# - Dedicated scripts are provided for datasets requiring:
#     * alternative station matching strategies
#     * specific SAR variable formats
#     * robust numeric conversion of pressure variables
...
for(file_path in all_files){
  ...
  if(length(station_sheet) == 0){
    ...
    if("station" %in% names(station_raw)){
      ...
      sar5_mapping <- list(
        ...
        "NS_NL_habiats_station.xlsx" = "pressure_value_5y",
  ...
  # ----------------------------
  # Load BESITO infauna lookup table and assign scores to prepared_df
  # ----------------------------
  # File paths
  ...
  if(!all(c("aphiaid","besito") %in% names(besito_infauna))){
    stop("The infauna file does not contain 'AphiaID' and 'BESITO' columns")
    }
