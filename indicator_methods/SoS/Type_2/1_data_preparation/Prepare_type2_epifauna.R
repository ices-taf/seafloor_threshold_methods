# =======================================================================
# Prepare Type 2 EPIFAUNA datasets for SoS calculation
# =======================================================================

# -----------------------------------------------------------------------
# Description
# -----------------------------------------------------------------------
# This script prepares Type 2 epifauna datasets for subsequent
...
# - Some Type 2 infauna datasets require special-case preparation scripts
#   due to differences in station identifiers, pressure format, or sheet structure
#
# -----------------------------------------------------------------------
# Required packages
# -----------------------------------------------------------------------
...
# -----------------------------------------------------------------------
# Exclude oxygen depletion dataset
# -----------------------------------------------------------------------

all_files <- all_files[!grepl("BS_southernbaltic_oxygendepletion\\.xlsx$", all_files)]
...
for(file_path in all_files){
  ...
  prepared_df <- prepared_df[rep(1, nrow(bio_raw)), ]
  
  # ----------------------------
  # Fill columns explicitly
  # ----------------------------
  ...
  if(length(missing_species_idx) > 0){
    ...
    prepared_df$Species <- ifelse(
      is.na(prepared_df$Species) | prepared_df$Species == "",
  ...
  if(length(station_sheet) == 0){
    ...
    if("station" %in% names(station_raw)){
      ...
      # ----------------------------
      sar5_mapping <- list(
        "BoBIC_GulfofCadizmud_btrawling.xlsx" = "pressure_value",
        "BoBIC_GulfofCadizsand_btrawling.xlsx" = "pressure_value"
      )
      sar5_info <- "Average over the last 5 years prior to the sample"
      
      sar1_mapping <- list(
        "NS_BfN_beam_btrawling.xlsx" = "pressure_value"
      )
      ...
      if("total_abundance" %in% names(station_raw)){
        prepared_df$Total_abundance <- station_raw$total_abundance[idx]
      }
  ...
  names(besito_epifauna) <- trimws(tolower(names(besito_epifauna)))
