...
load("C:/Users/jdepestele/OneDrive - ILVO/gitr/fbit_seafloor_quality_threshold_rbs/drafts/RNV_thr_scripts_JGHiddink/threshold_depth_longevity_model.RData")

out_folder <- "C:/Users/jdepestele/OneDrive - ILVO/gitr/fbit_seafloor_quality_threshold_rbs/output"
...
B_dt[, medlong := f_predlong(lintercept, lslope)]

saveRDS(B_dt, "./indicator_methods/LonglivedBiomass/medlong_by_station.RDS")
...
# # Two datasets have replicates. Replicates are averaged and not considered in the analysis
table(B_dt$.id, B_dt$replicate)
dim(B_dt)
numcols   <- names(B_dt)[sapply(B_dt, is.numeric)]
groupcols <- names(B_dt)[!sapply(B_dt, is.numeric) & names(B_dt) != "replicate"] # "replicate" is not a numeric column, but should be excluded from the grouping
B_dt <- B_dt[, lapply(.SD, mean, na.rm = TRUE),
             by = groupcols,
             .SDcols = numcols]
B_dt[, SEL_THR := round(SEL_THR, 8)]
# dim(B_dt)
