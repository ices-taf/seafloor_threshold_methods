...
selcols <- c(".id","gear_cat","pressure_type","long_Ksen")
...
B_pred <- B_dt[pressure_type %in% c("SAR", "trawl_index", "O2_depl") & !(.id %in% sel_id_multi_pres),
               {
                 ...
                 # CI of SAR threshold?
                 library(boot)
                 inv_pred <- function(data, indices) {
                   d <- data[indices, ]
                   m_boot <- glm(log(biom_Ksens + 1e-6) ~ pressure_value,
                                 family = gaussian,
                                 data = d)
                   b <- coef(m_boot)
                   log(unique(SEL_THR)) / b[2] # THRESHOLD MUST BE SPECIFIC BY .id, gear_cat, pressure_type
                   }
                 boot_res <- boot(data = data.frame(biom_Ksens     = biom_Ksens,
                                                    pressure_value = pressure_value),
                                  statistic = inv_pred, R = 1000)
                 ci <- boot.ci(boot_res, type = "bca")
                 SAR_at_THR_lower <- ci$bca[4]
                 SAR_at_THR_upper <- ci$bca[5]
...
selcols <- c(".id","pressure_type","resp_slope","pval", "SAR_at_THR_lower", "SAR_at_THR_pred", "SAR_at_THR_upper")
to_table2[pval>=0.05, 
          c("SAR_at_THR_lower", "SAR_at_THR_pred", "SAR_at_THR_upper") := NA]
to_table2 <- to_table2[order(pval) ,..selcols]
to_table2 <- thr_dt[to_table2, on=".id"]
...
ggplot(B_pred2[((pressure_type %in% c("SAR") & pressure_value < ceiling(max_pres)+0.5) | # ,"O2_depl"
  ...
  geom_ribbon(aes(ymin = y_lower, ymax = y_upper), fill = "firebrick4", alpha = 0.2) +
  ...
  labs(title = "Log-normal Regression with 95% CI",
       y = "Ratio of Longlived biomass fraction in sample \nto mean over reference samples",
...
#
save(B_dt, B_pred, B_pred2, file = "./output/Biomass_inoutput_model.Rdata")


```

The longlived biomass in each samples was estimated from the longevity of the top 10 % long-lived biomass fraction in low-disturbed samples, and the sample-specific cumulative longevity biomass distributions. The relative longlived biomass fractions were estimated by dividing longlived biomass with the mean longlived biomass fraction of the low-disturbed samples. The pressure-state relationships were estimated by fitting a log-normal model with the logarithm of relative long-lived biomass as a linear function of pressure: $\log(\text{RelativeLonglivedBiomass}) = \beta \cdot \text{pressure} + \varepsilon$ with $\varepsilon_i \sim \mathcal{N}(0, \sigma^2)$.
...
SAR-levels associated with case-specific threshold on longlived biomass were estimated using the inverted fitted relationship $\text{Pressure}_{\text{thr}} = \log(\text{Biomass}_{thr}) / \beta$. To quantify uncertainty around the estimated pressure value at which the biomass threshold was reached, a nonparametric bootstrap procedure was applied. For each combination of site (`.id`), gear category, and pressure type, the original dataset was resampled with replacement 1,000 times. For each bootstrap replicate, a generalized linear model with Gaussian errors was refitted on the log-transformed response. From each fitted model, the slope parameter $\beta$ was extracted and used to compute the pressure value corresponding to the predefined biomass threshold (*Type 1* data) by inverting the fitted relationship $\text{Pressure}_{\text{thr}} = \log(\text{Biomass}_{thr}) / \beta$. This yielded an empirical bootstrap distribution of threshold pressure estimates. Bias-corrected and accelerated (BCa) 95 % confidence intervals were then calculated from this distribution using the `boot` package.


```{r, include=T}
#| label: tbl-PresStat_SAR
#| tbl-cap: 'Slopes of pressure-state curves and pressures/SAR thresholds inferred from longlived biomass thresholds (current thresholds are random, but they should come from Type-1 data)'
#| tbl-pos: H
...
selcols <- c(".id","station_name", "biom_Ksens", "rel_biom_Ksens")
to_transfer <- B_dt
